...
предварительно стирая таблицу для бэкапа, 
...
CREATE OR REPLACE FUNCTION backup_customers() RETURNS void AS $$
    ...
    CREATE TABLE backedup_customers AS -- Рекомендованный способ!
...
CREATE OR REPLACE FUNCTION get_avg_frieght() RETURNS float8 AS $$
    SELECT avg (freight)
    FROM orders;
$$ LANGUAGE SQL;
SELECT * FROM get_avg_frieght();
...
/*4. Создать функцию, которая возвращает самые 
низкую и высокую зарплаты среди сотрудников заданного города*/
CREATE OR REPLACE FUNCTION get_salary_bounds_by_city(
...
при этом верхний уровень зарплаты по умолчанию равен 70, 
...
чтобы она возвращала только колонки last_name, first_name, title, salary*/
CREATE OR REPLACE FUNCTION correct_salary(upper_boundary numeric DEFAULT 70, correction_rate numeric DEFAULT 0.15) 
RETURNS TABLE(last_name text, first_name text, title text, salary numeric) employees AS 
$$
    UPDATE employees
    SET salary = salary + (salary * correction_rate)
    WHERE salary <= upper_boundary
    RETERNING last_name, first_name, title, salary;
$$ 
LANGUAGE SQL;
SELECT * FROM correct_salary();


/*8. Написать функцию, которая принимает метод доставки и 
возвращает записи из таблицы orders в которых freight меньше значения, 
...
CREATE OR REPLACE FUNCTION get_order_by_shipping(ship_method int) RETURNS SETOF orders AS
$$
DECLARE
    average numeric;
    maximum numeric;
...
$$
LANGUAGE plpgsql;
SELECT * FROM get_order_by_shipping(1);
SELECT count(*) FROM get_order_by_shipping(1);
...
минимальную зарплату (по умолчанию 30), 
коээфициет роста зарплаты (по умолчанию 20%)

Если зарплата выше минимальной, то возвращает false

Если зарплата ниже минимальной, то увеличивает зарплату 
на коэффициент роста и проверяет не станет ли зарплата после повышения превышать максимальную.
